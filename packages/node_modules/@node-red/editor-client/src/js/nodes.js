            ...
            if (n.wires) {
                for (var w1=0;w1<n.wires.length;w1++) {
                    ...
                    for (var w2=0;w2<wires.length;w2++) {
                        var wiresId;
                        var targetPort;
                        if(typeof wires[w2] == "string") {
                            wiresId = wires[w2];
                            targetPort = 0;
                        } else {
                            wiresId = wires[w2].id;
                            targetPort = wires[w2].port;
                        }
                        if (node_map.hasOwnProperty(wiresId)) {
                            if (n.z === node_map[wiresId].z) {
                                var link;
                                    link = {source: n, sourcePort: w1, target: node_map[wiresId], targetPort: targetPort};
                                addLink(link);
                                new_links.push(link);
                            } else {
                                console.log("Warning: dropping link that crosses tabs:",n.id,"->",node_map[wiresId].id);
                            }
                        }
                    }
                }
